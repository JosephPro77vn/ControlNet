...
def train(args):
    ...
    with open(args.config_path, 'r') as file:
        try:
            ...
            print(exc)
    print(config)
    wandb.init(project="control-train",
               config=config,
               name="vehicle-bottle")
    ...
    wandb.config.update({
        ...
        "ldm": config['ldm_params'],
        "autoencoder": config['autoencoder_params'],
        "training": config['train_params']
    })
    ...
    wandb.log({
        "example_images": wandb.Image(images[0].cpu()),
        "example_hint": wandb.Image(hint[0].cpu())
    })
    print(images.shape)
    print(hint.shape)
    print(input)
    ...
    for epoch_idx in range(num_epochs):
        ...
        for data in tqdm(data_loader):
            ...
            with torch.no_grad():
                ...
                # print("before encoded", im)
                # print("Noise shape", encoder_noise.shape)
                im = encoder(im, encoder_noise)
                # print("after encoded image", im)
            if 'text' in condition_types:
                with torch.no_grad():
            ...
            t = torch.clamp(t, 0, diffusion_config['num_timesteps']- 1)
            # print("time ", t.shape,  t)


            tokens = tokenizer.batch_encode_plus(
                prompt, padding="max_length", max_length=77
                # should have been text in empty String if there is text input
            ).input_ids
            # (Batch_Size, Seq_Len)
            ...
            # print("noisy image", noisy_im.shape, noisy_im)
            ...
            if step_count % 100 == 0:
                ...
                wandb.log({"gradient_norm": np.mean(gradients)}, step=step_count)
            ...
            # print(f"Params: {total_params:,}")  # Should match known values
            # print(list(model.state_dict().keys())[:1000])
            # print("Total keys:", len(list(model.state_dict().keys())))
            for name, param in model.named_parameters():
                if param.grad is not None:
        ...
        wandb.log({"epoch/loss": epoch_loss}, step=epoch_idx)
        print(f'Finished epoch:{epoch_idx + 1} | Loss : {epoch_loss:.4f}')
